...
from volcano_sdk import Session, SessionChangedError
from volcano_sdk._function_requests import FunctionAuth
from volcano_sdk._function_values import function_payload, header
...
def key_client(handler: Callable[[httpx.Request], httpx.Response]) -> InspectedClient:
    return InspectedClient(
        anon_key="anon-key",
...
def test_key_binding_rejects_a_session_installed_before_dispatch() -> None:
    client = InspectedClient(anon_key="anon-key")
    auth = FunctionAuth(client.context)
    session = Session(access_token("new"), "new-refresh", USER_ID)
    _ = client.auth.set_session(session)
    tokens: list[str] = []
...
def testfunction_payload_rejects_non_mappings(payload: object) -> None:
    with pytest.raises(TypeError, match="Function payload must be a mapping"):
        _ = function_payload(payload)
...
def testfunction_payload_rejects_non_json_values(payload: object) -> None:
...
def test_function_header_without_a_header_mapping_is_absent() -> None:
    assert header(None, "X-Volcano-Function-Invoked") is None
