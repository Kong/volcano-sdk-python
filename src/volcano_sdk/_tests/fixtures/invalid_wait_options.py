...
def non_callable_predicate() -> WaitUntilOptions:
    ...
    return WaitUntilOptions(until=None, initial_state=False)  # type: ignore[arg-type]  # pyright: ignore[reportArgumentType, reportReturnType]


def invalid_wait_duration(context: DurableContext, duration: object) -> None:
    context.wait("cool-off", duration)  # type: ignore[arg-type]  # pyright: ignore[reportArgumentType]
