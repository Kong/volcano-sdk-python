...
from volcano_sdk import AuthenticationError, Session, SessionChangedError
from volcano_sdk import _auth_base as auth_module
from volcano_sdk._auth_values import user_from_payload
...
def test_profile_commit_rechecks_ownership_after_parsing(
    ...
    monkeypatch.setattr(auth_module, "user_from_payload", parse_and_replace)
...
def test_session_request_requires_credentials_before_running_the_operation() -> None:
    ...
    with pytest.raises(RuntimeError, match="No active session"):
        _ = client.requests.request(operation)
...
def test_refresh_adopts_an_already_completed_refresh_without_rotating_again() -> None:
    ...
    result = client.requests.perform_refresh(binding, binding[2], notifications)
...
def test_rejected_refresh_defers_sign_out_notification_until_owner_unwinds() -> None:
    ...
    with pytest.raises(AuthenticationError):
        _ = client.requests.refresh_with_recovery(
...
def test_stale_refresh_binding_after_local_sign_out_is_session_changed() -> None:
    ...
    binding = client.capture_session_binding()
    assert client.clear_session_if_current(binding[0], event="SIGNED_OUT")

    with pytest.raises(SessionChangedError):
        _ = client.requests.owned_session(binding)
...
def test_empty_captured_sign_out_has_no_work_or_notifications() -> None:
    ...
    client.requests.sign_out_captured(binding, None, notifications, pending=False)

    assert client.current_session is None
    assert client.capture_session_binding() == binding
    assert notifications == []
...
def test_revocation_preserves_refresh_failure_without_an_access_session() -> None:
    ...
    with pytest.raises(AuthenticationError) as caught:
        client.requests.revoke_session(
...
def test_joined_revocation_does_not_start_another_refresh_after_401() -> None:
    ...
    with pytest.raises(AuthenticationError) as caught:
        client.requests.revoke_access_session(original, SESSION_A, None, joined=True)
