...
from volcano_sdk._client_session import (
...
class ExtraCredentials(BootstrapCredentials):
...
def test_bootstrap_rejects_extra_keys_in_a_structural_credentials_subtype() -> None:
    ...
    with pytest.raises(
        ...
        _ = bootstrap_session(credentials)
...
def test_profile_update_cannot_populate_an_absent_session() -> None:
    ...
    generation, session = client.capture_session()
    assert isinstance(generation, int)
    assert session is None

    assert not client.update_session_user_if_current({"id": "user"}, generation)
...
def test_callback_dispatch_state_has_boolean_ownership_and_empty_failure() -> None:
    ...
    assert client.dispatching_auth_notifications is False
    events: list[str] = []
...
def test_refresh_commit_rejects_a_changed_user_before_replacing_credentials() -> None:
    ...
    assert client.capture_session()[0] == generation
