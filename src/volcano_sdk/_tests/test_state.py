...
class DataTransport(OAuthTransport):
    def __init__(self) -> None:
        super().__init__()
...
@final
class StateTransport(DataTransport):
...
def test_profile_operations_update_the_local_snapshot(
    ...
    client = InspectedClient(anon_key="anon", _transport=StateTransport())
    ...
    assert client.capture_session_binding()[:2] == binding[:2]
    assert events == []
...
def test_profile_operations_preserve_equivalent_session_user_ids(
    ...
    client = InspectedClient(anon_key="anon", _transport=transport)
...
def test_auth_session_binding_preserves_lineage_across_token_refresh() -> None:
    ...
    client = InspectedClient(anon_key="anon", _transport=transport)
    _ = client.auth.sign_in(email="user@example.com", password="secret")
    generation, lineage, _session = client.capture_session_binding()

    refreshed = client.auth.refresh_session()
    next_generation, next_lineage, captured = client.capture_session_binding()
