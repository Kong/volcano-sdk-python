...
class FakeDatabaseTransport(FakeStateTransport):
...
class FakeStorageTransport(FakeStateTransport):
...
class FakeLockTransport(FakeStateTransport):
...
class FakeTransport(FakeDatabaseTransport, FakeStorageTransport, FakeLockTransport):
...
class BoundedNonSeekableReader(RawIOBase):
    def __init__(self, value: bytes) -> None:
        super().__init__()
...
class TemporarilyUnavailableReader(RawIOBase):
    def __init__(self) -> None:
        super().__init__()
        self.read_sizes: list[int] = []
...
def test_storage_list_normalizes_an_empty_terminal_cursor() -> None:
    ...
    prefix = transport.calls[-1][1]["prefix"]
