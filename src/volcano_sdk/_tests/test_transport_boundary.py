...
from volcano_sdk._transport_response import (
    generated_request,
    json_object,
    parsed_response,
    response_payload,
...
def testgenerated_request_preserves_valid_fields() -> None:
    ...
    assert not response.request.url.params["cursor"]
    assert response.request.content == b'{"value":1}'


def testgenerated_request_accepts_read_only_header_and_query_mappings() -> None:
    with _client() as client:
        response = generated_request(
            client,
            {
...
def testjson_object_rejects_non_string_keys() -> None:
    response = _InvalidKeyResponse(200)
    with pytest.raises(TypeError) as caught:
        _ = json_object(response)
    assert str(caught.value) == "Invalid generated request field: response body key"


def testjson_object_rejects_a_non_object_response() -> None:
    with pytest.raises(TypeError) as caught:
...
def test_response_payload_keeps_error_message_precedence_and_code(
    ...
    response = GeneratedTransportResponse(422, payload, b"", {})
...
def test_response_payload_classifies_the_last_server_error_status() -> None:
    response = GeneratedTransportResponse(599, {}, b"", {})
