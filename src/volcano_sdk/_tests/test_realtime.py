...
from .fixtures.invalid_arguments import (
    ...
    remove_unsupported_realtime_channel_type,
...
class RecordedFactoryArguments(TypedDict):
    address: str
    events: object
    token: str
...
@dataclass
class FakePublicationContext:
    pub: realtime_module.Publication
...
def _command_section(command: dict[str, object], name: str) -> dict[str, object]:
    value = command[name]
    if not _is_string_keyed_dict(value):
        message = f"invalid native {name} command"
        raise AssertionError(message)
    return value


def _ignore_presence_state(_state: Mapping[str, RealtimePresenceInfo]) -> None:
    return


def _record_channel_hit(received: list[str], name: str) -> Callable[[object], None]:
    def record(_data: object) -> None:
...
def centrifuge_error(message: str) -> Exception:
    return CentrifugeError(message)
...
def test_realtime_fetches_session_bound_postgres_rows() -> None:
    ...
    assert asyncio.run(
        realtime_state(client.realtime).fetch_postgres_rows((request,))
    ) == ({"id": 42, "body": "fetched"},)
...
def test_realtime_fetch_requires_async_select_transport() -> None:
    ...
    with pytest.raises(TypeError, match="does not support realtime Postgres row fetch"):
        _ = asyncio.run(realtime_state(client.realtime).fetch_postgres_rows((request,)))
...
class FakeSubscription:
    def __init__(
        ...
        *,
        ...
        if events is not None and not isinstance(events, ChannelEvents):
            message = "expected SDK channel events"
            raise TypeError(message)
        self.name: str = name
        self.events: ChannelEvents | None = events
        self.join_leave: bool = join_leave
        self.recoverable: bool = recoverable
        self.calls: list[tuple[str, object]] = []
        self.presence_clients: dict[str, object] = {}
        ...
        self.inside_subscribed_handler: bool = False
    ...
    def _events(self) -> ChannelEvents:
        if self.events is None:
            message = "detached test subscription has no events"
            raise AssertionError(message)
        return self.events
    ...
    async def emit_subscribed(self) -> None:
        ...
        try:
            await self._events().on_subscribed(
                SimpleNamespace(
                    channel=self.name,
    ...
    async def emit_subscribing(self) -> None:
        ...
        await self._events().on_subscribing(
            SimpleNamespace(code=0, reason="reconnecting")
        )

    async def emit(self, data: object) -> None:
        await self._events().on_publication(
            FakePublicationContext(pub=FakePublication(data=data))
        )
    ...
    async def emit_join(self, info: object) -> None:
        ...
        if not isinstance(client, str):
            ...
            raise TypeError(message)
        self.presence_clients[client] = info
    ...
    async def emit_leave(self, info: object) -> None:
        ...
        if not isinstance(client, str):
            message = "expected presence client ID"
            raise TypeError(message)
        _ = self.presence_clients.pop(client, None)
        await self._events().on_leave(SimpleNamespace(info=info))


class FakeCentrifugeClient:
    def __init__(self, events: object = None) -> None:
        self.calls: list[str] = []
        self.events: ClientEvents | None = None
        self.set_events(events)
        self.state: FakeConnectionState = FakeConnectionState(value="disconnected")
        self.presence_clients: dict[str, object] = {}
        self.presence_error: Exception | None = None
        self.presence_entered: asyncio.Event | None = None
        self.presence_release: asyncio.Event | None = None
    ...
    def set_events(self, events: object) -> None:
        if events is not None and not isinstance(events, ClientEvents):
            message = "expected SDK connection events"
            raise TypeError(message)
        self.events = events

    async def connect(self) -> None:
        self.calls.append("connect")
        self.state = FakeConnectionState(value="connected")
    ...
    async def disconnect(self) -> None:
        ...
        self.state = FakeConnectionState(value="disconnected")
        self._subs.clear()
        if self.events is not None:
    ...
    async def emit_wire_publication(self, name: str, data: object) -> None:
...
class BlockingConnectCentrifugeClient(FakeCentrifugeClient):
    def __init__(self) -> None:
        super().__init__()
        self.connect_started: asyncio.Event = asyncio.Event()
        self.connect_release: asyncio.Event = asyncio.Event()
...
class SDKNativeClient(NativeClient):
    ...
    def remove_subscription(self, subscription: object) -> None:
        if not isinstance(subscription, Subscription):
            message = "expected native subscription"
            raise TypeError(message)
...
async def test_realtime_callback_errors_are_empty_after_native_presence_reply() -> None:
    ...
    subscription = FakeSubscription(
        channel.name,
        ChannelEvents(channel_state(channel)),
        join_leave=True,
        recoverable=True,
    )
    subscription.presence_entered = asyncio.Event()
    channel_state(channel).subscription = subscription
    channel_state(channel).subscribed = True
    loop = asyncio.get_running_loop()
    ...
    sync = asyncio.create_task(channel_state(channel).presence.run_presence_sync())
    try:
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            await asyncio.wait_for(
                channel_state(channel).callback_queue.join(), timeout=1
            )
...
def test_realtime_native_dispatch_routes_user_scoped_postgres_publications(
    ...
    async def scenario() -> None:
        ...
        def on_insert(change: PostgresChange) -> None:
        ...
        try:
            ...
            _ = await asyncio.wait_for(delivered.wait(), timeout=0.2)
...
async def start_native_presence_refresh(
    ...
    await subscribing
    await channel_state(channel).presence.wait_presence_sync()
...
def test_realtime_stopped_presence_query_accepts_late_native_reply(
    ...
    async def scenario() -> None:
        ...
        def mark_received(_message: object) -> None:
            received.set()

        _ = healthy.on("message", mark_received)
...
def test_realtime_cancelled_pause_keeps_native_subscription_resumable(
    ...
    async def scenario() -> None:
        ...
        assert_same(channel_state(channel).subscribed, expected=False)
        ...
        try:
            ...
            await resuming
            await channel_state(channel).presence.wait_presence_sync()
            assert_same(channel_state(channel).subscribed, expected=True)
            assert factory.subscription(channel.name) is subscription
        finally:
            await client.realtime.disconnect()
            _ = await asyncio.gather(resuming, return_exceptions=True)
...
def test_realtime_repeated_pause_retains_presence_clear_notification(
    ...
    async def scenario() -> None:
        ...
        async def receive(state: Mapping[str, RealtimePresenceInfo]) -> None:
...
def test_realtime_postgres_delivery_identity_changes_on_reauthentication() -> None:
    ...
    async def scenario() -> None:
        ...
        assert channel_state(channel).postgres_delivery_is_current(identity)

        _ = client.auth.sign_in(email="user@example.com", password="secret")

        identity_after_reauthentication = channel_state(
            channel
        ).capture_postgres_delivery_identity()
        assert not channel_state(channel).postgres_delivery_is_current(identity)
        assert not channel_state(channel).postgres_delivery_is_current(
...
def test_realtime_postgres_delivery_identity_changes_on_resubscription() -> None:
    ...
    async def scenario() -> None:
        ...
        next_identity = channel_state(channel).capture_postgres_delivery_identity()

        assert not channel_state(channel).postgres_delivery_is_current(identity)
        assert channel_state(channel).postgres_delivery_is_current(next_identity)
        await client.realtime.disconnect()
...
def test_realtime_rejects_new_subscriptions_after_session_changes() -> None:
    ...
    async def scenario() -> None:
        ...
        assert channel_state(postgres).subscription is None
        await client.realtime.disconnect()
...
async def test_realtime_drops_queued_postgres_callbacks_from_an_old_epoch() -> None:
    ...
    def insert(record_id: int) -> dict[str, object]:
        return {
            "type": "INSERT",
            "schema": "public",
            "table": "messages",
    ...
    async def on_insert(change: PostgresChange) -> None:
    ...
    first_worker = channel_state(channel).postgres_worker
    assert first_worker is not None
    await subscription.emit(insert(2))

    await subscription.emit_subscribing()
    assert_same(channel_state(channel).postgres_worker, expected=None)
    release_first.set()
    await subscription.emit_subscribed()
    await subscription.emit(insert(3))
    _ = await asyncio.wait_for(third_received.wait(), timeout=0.2)

    assert received == [1, 3]
    assert channel_state(channel).postgres_worker is not first_worker
    await client.realtime.disconnect()
...
def test_realtime_only_queues_changes_with_an_interested_listener() -> None:
    ...
    async def scenario() -> None:
        ...
        class UnhashableListener:
            ...
            def __call__(self, change: PostgresChange) -> None:
                self.changes.append(change)
                self.received.set()

        def publication(event: str, table: str) -> dict[str, object]:
            return {
                "type": event,
                "schema": "public",
                "table": table,
        ...
        filtered: list[PostgresChange] = []
        unfiltered: list[PostgresChange] = []
        ...
        _ = channel.on("*", UnhashableListener(unfiltered, unfiltered_received))
        await subscription.emit(publication("UPDATE", "other"))
        _ = await asyncio.wait_for(unfiltered_received.wait(), timeout=0.2)

        assert filtered == []
        assert len(unfiltered) == 1
        assert channel_state(channel).postgres_worker is not None
        await client.realtime.disconnect()
...
def test_realtime_captures_supported_postgres_fetch_request() -> None:
    ...
    async def scenario() -> None:
        ...
        request = channel_state(channel).postgres_fetch_request(change)
        client.realtime.set_database_name("next")

        assert request == PostgresFetchRequest(
            database_name="app",
            access_token="access-1",
            table="messages",
            row_id=42,
        )
        assert channel_state(channel).postgres_fetch_request(
        ...
        assert (
            channel_state(channel).postgres_fetch_request(
                realtime_module.PostgresChange(
                    type="DELETE",
                    schema="public",
                    table="messages",
        ...
        client.realtime.set_database_name(None)
        assert channel_state(channel).postgres_fetch_request(change) is None
        await client.realtime.disconnect()
...
def test_realtime_delivers_lightweight_fallback_when_row_is_absent() -> None:
    ...
    async def scenario() -> None:
        ...
        def on_update(change: PostgresChange) -> None:
            changes.append(change)
            received.set()
...
def test_realtime_unsubscribe_does_not_wait_for_obsolete_row_fetches() -> None:
    ...
    async def scenario() -> None:
        ...
        _ = await asyncio.wait_for(transport.started.wait(), timeout=0.2)
...
def test_realtime_disconnect_invalidates_channels_before_clearing_auth() -> None:
    ...
    with pytest.raises(RuntimeError, match="no session binding"):
        _ = realtime_state(client.realtime).connection_token()
    ...
    async def scenario() -> None:
        ...
        def observe_disconnect_boundary() -> None:
            nonlocal observed
            assert_same(channel_state(channel).subscribed, expected=False)
            assert realtime_state(client.realtime).connection_token() == "access-1"
            observed = True
...
def test_realtime_routes_immutable_rls_scoped_postgres_changes() -> None:
    ...
    async def scenario() -> None:
        updates: list[PostgresChange] = []
        inserts: list[PostgresChange] = []
        received = asyncio.Event()
        ...
        _ = await asyncio.wait_for(received.wait(), timeout=0.1)
        ...
        with pytest.raises(TypeError):
            cast("dict[str, object]", change.record)["body"] = "mutated"
...
async def test_realtime_drops_malformed_records_before_delivering_valid_changes(
    ...
    try:
        ...
        _ = await asyncio.wait_for(received.wait(), timeout=1)
...
def test_realtime_normalizes_lightweight_postgres_deletes() -> None:
    ...
    async def scenario() -> None:
        ...
        def on_delete(change: PostgresChange) -> None:
            changes.append(change)
            if len(changes) == 2:
                received.set()
...
def test_realtime_validates_postgres_change_operations() -> None:
    ...
    with pytest.raises(ValueError, match="only available for postgres"):
        _ = broadcast.on_postgres_changes(
            "*",
            schema="public",
            table="messages",
            callback=lambda _change: None,
    ...
    with pytest.raises(ValueError, match="unsupported Postgres change event"):
        unsupported_postgres_change_event(postgres)
...
def test_realtime_presence_sync_tracks_initial_join_and_leave_state() -> None:
    ...
    async def scenario() -> None:
        channel = client.realtime.channel("lobby", channel_type="presence")
        sync_states: list[Mapping[str, RealtimePresenceInfo]] = []
        joins: list[RealtimePresenceInfo] = []
        leaves: list[RealtimePresenceInfo] = []
        synced = asyncio.Event()

        def on_sync(state: Mapping[str, RealtimePresenceInfo]) -> None:
            sync_states.append(state)
            synced.set()

        stop_sync = channel.on_presence_sync(on_sync)
        _ = channel.on("join", joins.append)
        _ = channel.on("leave", leaves.append)
        ...
        _ = await asyncio.wait_for(synced.wait(), timeout=0.1)
        assert channel.get_presence_state() == {
...
def test_realtime_presence_resync_replays_concurrent_join_and_leave() -> None:
    ...
    async def scenario() -> None:
        ...
        official.subscription.presence_release.set()
        _ = await asyncio.gather(resync, join, leave)
...
def test_realtime_unsubscribe_clears_presence_state() -> None:
    ...
    async def scenario() -> None:
        channel = client.realtime.channel("lobby", channel_type="presence")
        states: list[Mapping[str, RealtimePresenceInfo]] = []
        ...
        await channel.unsubscribe()
        _ = await asyncio.wait_for(cleared.wait(), timeout=0.1)
...
async def test_realtime_presence_sync_coalesces_latest_backpressured_state() -> None:
    ...
    async def on_sync(state: Mapping[str, RealtimePresenceInfo]) -> None:
...
def test_realtime_newer_queued_snapshot_discards_older_pending_snapshot() -> None:
    ...
    async def scenario() -> None:
        channel = client.realtime.channel("lobby", channel_type="presence")
        _ = channel.on_presence_sync(_ignore_presence_state)
        ...
        for _ in range(channel_state(channel).callback_queue.maxsize):
        ...
        await channel_state(channel).emit("presence_sync", {"version": 1})
        _ = channel_state(channel).callback_queue.get_nowait()
        channel_state(channel).callback_queue.task_done()
        await channel_state(channel).emit("presence_sync", {"version": 2})
        queued: list[object] = []
        while not channel_state(channel).callback_queue.empty():
            queued.append(channel_state(channel).callback_queue.get_nowait().data)
            channel_state(channel).callback_queue.task_done()
...
def test_realtime_rejects_unsupported_channel_types() -> None:
    ...
    with pytest.raises(ValueError, match="channel type"):
        unsupported_realtime_channel_type(client.realtime)

    async def scenario() -> None:
        with pytest.raises(ValueError, match="channel type"):
            await remove_unsupported_realtime_channel_type(client.realtime)
...
def test_realtime_reports_presence_query_failures() -> None:
    ...
    async def scenario() -> None:
        ...
        _ = client.realtime.on_error(on_error)
        channel = client.realtime.channel(
            "lobby",
...
def test_realtime_wraps_official_client_without_exposing_it() -> None:
    ...
    official = FakeCentrifugeClient()
    factory_arguments: list[RecordedFactoryArguments] = []
    ...
    async def scenario() -> None:
        ...
        generation, _lineage, _session = client.capture_session_binding()
        refreshed = Session(
            access_token="access-2",
            refresh_token="refresh-token-2",
            user_id="user-123",
        )
        assert client.set_session_if_current(
            refreshed,
            generation,
            event="TOKEN_REFRESHED",
        )
        assert factory_arguments
        assert await factory_arguments[0]["get_token"]() == "access-2"
        transport.access_token = "access-3"
        _ = client.auth.sign_in(email="user@example.com", password="secret")
        with pytest.raises(RuntimeError, match="session changed"):
            _ = await factory_arguments[0]["get_token"]()
        assert realtime_state(client.realtime).connection_token() == "access-2"
        await client.realtime.disconnect()

    asyncio.run(scenario())

    assert len(factory_arguments) == 1
    assert factory_arguments[0]["address"] == (
        "wss://api.test.volcano.dev/realtime/v1/websocket?apikey=anon%20key"
    )
    assert factory_arguments[0]["token"] == "access-1"
...
def test_realtime_discards_queued_presence_events_on_pause(*, resume: bool) -> None:
    ...
    async def scenario() -> None:
        ...
        _ = channel.on("join", received.append).on("leave", received.append)
        _ = channel.on_presence_sync(snapshots.append)
        await channel.subscribe()
        await asyncio.wait_for(
            channel_state(channel).callback_queue.join(), timeout=0.2
...
def test_centrifuge_preserves_recovery_position_across_unsubscribe(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def scenario() -> None:
        monkeypatch.setattr(native_client_module, "Subscription", SDKNativeSubscription)
...
def test_realtime_retains_a_provisional_connection_when_cleanup_fails() -> None:
    ...
    async def scenario() -> None:
        ...
        with pytest.raises(type(cleanup_error), match="disconnect failed"):
            await subscribing
        assert realtime_state(client.realtime).connection is not None
...
def test_realtime_connection_callbacks_receive_immutable_contexts() -> None:
    ...
    async def scenario() -> None:
        ...
        _ = client.realtime.on_disconnect(disconnect_callback)
        stop_error = client.realtime.on_error(on_error)

        await client.realtime.channel("contract").subscribe()
        _ = await asyncio.wait_for(connected_event.wait(), timeout=0.1)
        ...
        _ = await asyncio.wait_for(error_event.wait(), timeout=0.1)
        assert errors == [
        ...
        stop_error()
        await client.realtime.disconnect()
        _ = await asyncio.wait_for(disconnected_event.wait(), timeout=0.1)
        assert disconnected == [
...
def test_realtime_connection_callbacks_do_not_block_transport_events() -> None:
    ...
    async def scenario() -> None:
        ...
        _ = await asyncio.wait_for(callback_started.wait(), timeout=0.1)
...
def test_realtime_connection_state_tracks_transport_disconnects() -> None:
    ...
    async def scenario() -> None:
        ...
        official.state = FakeConnectionState(value="connecting")
...
def test_realtime_continues_removing_channels_after_one_failure() -> None:
    ...
    async def scenario() -> None:
        ...
        error = centrifuge_error("unsubscribe failed")
        official.subscriptions["broadcast:first"].unsubscribe_error = error
...
def test_realtime_can_remove_all_channels_from_a_message_callback() -> None:
    ...
    async def scenario() -> None:
        ...
        async def remove_channels(_: object) -> None:
            await client.realtime.remove_all_channels()
            removed.set()

        _ = channel.on("message", remove_channels)
        await channel.subscribe()
        assert official.subscription is not None
        await official.subscription.emit({"event": "message"})

        _ = await asyncio.wait_for(removed.wait(), timeout=0.2)
        replacement = client.realtime.channel("contract")
        assert replacement is not channel
        await replacement.subscribe()
        await client.realtime.disconnect()
...
def test_realtime_removes_all_channels_without_disconnecting() -> None:
    ...
    async def scenario() -> None:
        ...
        await first.subscribe()
        await second.subscribe()
        subscriptions = tuple(official.subscriptions.values())
...
def test_realtime_callbacks_run_outside_the_message_processor() -> None:
    ...
    async def scenario() -> None:
        ...
        _ = await asyncio.wait_for(started.wait(), timeout=0.1)
        release.set()
        await client.realtime.disconnect()
...
def test_realtime_routes_overlapping_channel_suffixes_to_the_longest_match() -> None:
    ...
    async def scenario() -> None:
        ...
        short = client.realtime.channel("foo").on(
            "message", _record_channel_hit(received, "short")
        )
        long = client.realtime.channel("x:broadcast:foo").on(
            "message", _record_channel_hit(received, "long")
        )
...
def test_realtime_opens_one_connection_when_first_used_concurrently(
    ...
    def factory(*args: object, **kwargs: object) -> FakeCentrifugeClient:
        nonlocal created
        del args, kwargs
        created += 1
        return official
    ...
    async def scenario() -> None:
        first = asyncio.create_task(realtime_state(client.realtime).connect())
        _ = await asyncio.wait_for(entered.wait(), timeout=2)
        second = asyncio.create_task(realtime_state(client.realtime).connect())
        await asyncio.sleep(0)
        release.set()
...
def test_realtime_callback_can_disconnect_its_own_client() -> None:
    ...
    async def scenario() -> None:
        ...
        _ = await asyncio.wait_for(disconnected.wait(), timeout=0.1)
        await client.realtime.disconnect()
...
def test_realtime_disconnect_excludes_a_concurrent_first_connect(
    ...
    async def scenario() -> None:
        ...
        with pytest.raises(asyncio.CancelledError):
            await subscribing
        await disconnecting
        assert channel_state(channel).subscription is None
...
def test_realtime_disconnect_closes_transport_when_channel_reset_is_cancelled(
    ...
    async def scenario() -> None:
        ...
        second_reset = False
        original_second_reset = channel_state(second).reset
        ...
        monkeypatch.setattr(channel_state(first), "reset", blocking_reset)
        monkeypatch.setattr(channel_state(second), "reset", observe_second_reset)
        disconnecting = asyncio.create_task(client.realtime.disconnect())
        ...
        assert channel_state(first).subscription is None
        assert not channel_state(first).subscribed
        assert second_reset
        assert channel_state(second).subscription is None
        assert not channel_state(second).subscribed
        assert realtime_state(client.realtime).connection is None
        assert realtime_state(client.realtime).connection_access_token is None
        assert realtime_state(client.realtime).connection_session_lineage is None
...
def test_realtime_disconnect_excludes_subscription_on_an_existing_connection(
    ...
    def new_subscription(
        ...
        *,
        ...
        official.subscriptions[name] = official.subscription
...
def test_realtime_disconnect_snapshots_channels_before_resetting(
    ...
    async def scenario() -> None:
        ...
        entered = asyncio.Event()
        release = asyncio.Event()
        original_reset = channel_state(first).reset
...
def test_realtime_shutdown_does_not_cancel_or_wait_for_application_work(
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            await asyncio.sleep(0)
            assert not realtime_state(client.realtime).callback_tasks
        finally:
            release.set()
            await client.realtime.disconnect()
...
def test_realtime_callback_child_tasks_can_stop_delivery() -> None:
    ...
    async def scenario() -> None:
        ...
        async def receive(_message: object) -> None:
            _ = await asyncio.gather(client.realtime.remove_channel("room"))
...
def test_realtime_event_loop_shutdown_does_not_start_queued_callbacks() -> None:
    ...
    asyncio.run(scenario())
    assert received == ["running"]
    callback_task = channel_state(channel).callback_task
    assert callback_task is None or callback_task.done()
...
def test_realtime_worker_registration_precedes_callback_execution(
    ...
    async def scenario() -> None:
        ...
        def on_sync(_state: Mapping[str, RealtimePresenceInfo]) -> None:
            received.append("presence")
        ...
        await subscribing
        await channel_state(channel).presence.wait_presence_sync()
        await asyncio.wait_for(channel_state(channel).callback_queue.join(), timeout=1)
        await asyncio.sleep(0)
        received.clear()
        loop = asyncio.get_running_loop()
    ...
    async def scenario() -> None:
        ...
        try:
            for _ in range(2):
                requested.clear()
                subscribing = asyncio.create_task(channel.subscribe())
                _ = await asyncio.wait_for(requested.wait(), timeout=0.2)
                assert not subscribing.done()
                assert official.subscription is not None
                await official.subscription.emit_subscribed()
                await asyncio.wait_for(subscribing, timeout=0.2)
...
def test_realtime_subscribe_releases_connection_lock_after_readiness_failure(
    ...
    readiness_calls: list[FakeSubscription] = []

    async def fail_ready(subscription: FakeSubscription) -> None:
        readiness_calls.append(subscription)
        await failed_operation(error)

    monkeypatch.setattr(FakeSubscription, "ready", fail_ready)
    ...
    assert readiness_calls == [official.subscription]
...
async def test_realtime_removal_can_overlap_readiness_rollback(
    ...
    try:
        _ = await asyncio.wait_for(unsubscribed.wait(), timeout=0.2)
        release.set()
        ...
        release.set()
        _ = await asyncio.gather(subscribing, removing, return_exceptions=True)
        await client.realtime.disconnect()
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            if operation in {"remove", "disconnect"}:
                assert official.subscription is not None
                await official.subscription.emit_subscribed()
                assert not channel_state(pending).subscribed
                with pytest.raises(asyncio.CancelledError):
                    await asyncio.wait_for(subscribing, timeout=0.2)
            else:
                assert official.subscription is not None
            ...
            _ = subscribing.cancel()
            _ = await asyncio.gather(subscribing, return_exceptions=True)
            await client.realtime.disconnect()
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            if channel_type == "broadcast":
                ...
                command = await factory.command()
                assert _command_section(command, "publish")["data"] == "ready"
...
def test_realtime_native_failed_readiness_stops_late_acknowledgement_and_allows_retry(
    ...
    async def scenario() -> None:
        ...
        try:
            ...
            original_subscription = factory.subscription(channel.name)
            if cancelled:
                _ = subscribing.cancel()
...
def test_realtime_removed_channel_does_not_cancel_callback_subscription(
    ...
    async def scenario() -> None:
        ...
        try:
            command = await factory.command()
            assert _command_section(command, "unsubscribe")["channel"] == active.name
            await factory.reply(command, unsubscribe={})
            await asyncio.wait_for(removing, timeout=0.2)
            assert not completed.is_set()
            await factory.reply(pending_command, subscribe={})
            _ = await asyncio.wait_for(completed.wait(), timeout=0.2)
            assert channel_state(pending).subscribed
            assert client.realtime.channel("active") is not active
        finally:
            await client.realtime.disconnect()
            _ = await asyncio.gather(removing, return_exceptions=True)
...
def test_realtime_pause_during_readiness_retains_native_recovery(
    ...
    async def scenario() -> None:
        ...
        subscribing = asyncio.create_task(channel.subscribe())
        pending_command = await factory.command()
        assert _command_section(pending_command, "subscribe")["offset"] == 10
        pausing = asyncio.create_task(channel.unsubscribe())
        command = await factory.command()
        try:
            await factory.reply(pending_command, subscribe={})
            ...
            with pytest.raises(RuntimeError, match="subscription was interrupted"):
                await subscribing
            assert channel_state(channel).subscription is subscription
            subscribing = asyncio.create_task(channel.subscribe())
            command = await factory.command()
            assert _command_section(command, "subscribe")["offset"] == 10
            await factory.reply(command, subscribe={})
            await asyncio.wait_for(subscribing, timeout=0.2)
        finally:
            await client.realtime.disconnect()
            _ = await asyncio.gather(subscribing, pausing, return_exceptions=True)
    ...
    async def scenario() -> None:
        ...
        def factory(
            ...
            *,
            ...
            return next(factories)(
                address, events=events, token=token, get_token=get_token
            )
        ...
        try:
            ...
            if retrying is not None:
                _ = retrying.cancel()
                _ = await asyncio.gather(retrying, return_exceptions=True)
            _ = await asyncio.gather(subscribing, return_exceptions=True)
            await client.realtime.disconnect()
...
async def test_realtime_stop_invalidates_queued_subscribe_calls(
    ...
    def factory(
        ...
        *,
        ...
        return next(factories)(address, events=events, token=token, get_token=get_token)
...
def test_realtime_binds_bootstrap_refresh_without_profile(
    *, same_session: bool
    ...
    class BootstrapTransport(AuthTransport):
        @staticmethod
        def auth_refresh(**_arguments: object) -> Response:
            return Response(
                200,
                {
                    "access_token": token(
