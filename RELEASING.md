...
Install `kong-volcano-app` on `Kong/volcano-sdk-python`. Set repository variable
...
PyPI currently rejects the intended name `volcano-sdk`. Resolve that restriction
or approve a new name before registering a **pending trusted publisher**.
...
Release Please updates `pyproject.toml` and the SDK entry in `uv.lock`.
The TOML JSONPath uses the tagged `name.value` representation of the pinned
Release Please 17.6.0 updater. Retest that updater when upgrading Release Please;
`uv sync --frozen` must pass on its generated release PR.
